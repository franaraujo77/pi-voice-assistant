...
class LEDsEventHandler(AsyncEventHandler):
    ...
    async def handle_event(self, event: Event) -> bool:
        ...
        if Detection.is_type(event.type):
            ...
            if self.timeout_task and not self.timeout_task.done():
                self.timeout_task.cancel()
            # Reset streaming state and keep processing state for TTS
            self.streaming = False
    ...
    async def handle_timeout(self):
        ...
        try:
            # Wait 5 seconds after wake word
            await asyncio.sleep(5)
            if self.is_processing and not self.streaming:
                # If still processing but not streaming, timeout occurred
                self.color(_RED)
                await asyncio.sleep(2)  # Show red for 2 seconds
                self.reset_state()
        except asyncio.CancelledError:
            pass

    async def handle_no_text_error(self):
        """Handle the no text recognized error with red light."""
        try:
            self.color(_RED)
            await asyncio.sleep(2)  # Show red for 2 seconds
        except asyncio.CancelledError:
            ...
            self.reset_state()
            self.error_task = None  # Clear error task reference
    ...
    def reset_state(self):
        """Reset all state flags and turn off LEDs."""
        self.is_processing = False
        self.streaming = False
        self.color(_BLACK)
